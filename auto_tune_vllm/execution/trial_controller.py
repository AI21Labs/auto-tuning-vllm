...
class BaseTrialController(TrialController):
    ...
    def _handle_benchmark_running(
        ...
        max_benchmark_time = trial_config.benchmark_config.max_seconds * 3
    ...
    def cleanup_resources(self):
        ...
        if self.vllm_process:
            pid = self.vllm_process.pid
            controller_logger.info(
                f"Trial Controller: Cleaning up vLLM server process " f"(PID: {pid})..."
            )
            self._flush_logger_handlers(controller_logger)
            ...
            try:
                ...
                controller_logger.warning(
                    f"Trial Controller: Failed to get process group id " f"for {pid}"
                )
                pgid = None
